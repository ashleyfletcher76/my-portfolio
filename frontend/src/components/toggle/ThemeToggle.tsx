...
export default function ThemeToggle() {
  ...
  return (
    ...
    >
      {/* Icons */}
      <FaSun
        className={`pointer-events-none absolute left-1.5 z-10 h-4 w-4 text-amber-700 transition-opacity duration-200 dark:text-amber-100 ${isDark ? 'opacity-50' : 'opacity-100'} `}
        aria-hidden
      />
      <FaMoon
        className={`pointer-events-none absolute right-1.5 z-10 h-4 w-4 text-slate-600 transition-opacity duration-200 dark:text-indigo-200 ${isDark ? 'opacity-100' : 'opacity-50'} `}
        aria-hidden
      />
